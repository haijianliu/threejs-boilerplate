...
float line(in vec2 pos, in float l){
  return  (
    smoothstep(-lLength-smoother,-lLength,pos.x)
    -smoothstep(lLength,lLength+smoother,pos.x)
    )*(
    smoothstep(-lineBord-smoother,-lineBord,pos.y)
    -smoothstep(lineBord,lineBord+smoother,pos.y)
    );
}
...
float twoDots(in vec2 pos, float y, float delay){
  lLength = 0.05;
  rDot = 0.01;
  lLength *= sin(0.5*PI*(time+delay));
  pos -= vec2(lLength,-y);
  dot1 = round(pos,rDot*(1.0+0.2*cos(0.5*PI*(time+delay))));
  pos += vec2(lLength+lLength,0.0);
  dot2 = round(pos,rDot*(1.0+0.2*cos(0.5*PI*(time+2.0+delay))));
